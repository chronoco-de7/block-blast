...
function render() {
    ...
    // Clear canvas (darker base for neon contrast)
    boardCtx.fillStyle = '#060a12';
    boardCtx.fillRect(0, 0, boardCanvas.width, boardCanvas.height);
    
    // Draw grid (glowing cyan lines)
...
// Draw a single cell (glossy + neon glow)
function drawCell(ctx, x, y, color) {
    ...
    highlight.addColorStop(0.35, 'rgba(255, 255, 255, 0.06)');
...
// Draw shape (preview blocks - glossy + glow)
                    ...
                    for (let col = 0; col < cols; col++) {
                        if (matrix[row][col] === 1) {
                            ...
                            if (row === pivot.row && col === pivot.col) {
                                boardCtx.strokeStyle = 'rgba(0, 230, 255, 0.95)';
                                boardCtx.lineWidth = 3;
                            } else {
                                boardCtx.strokeStyle = 'rgba(0, 220, 255, 0.7)';
